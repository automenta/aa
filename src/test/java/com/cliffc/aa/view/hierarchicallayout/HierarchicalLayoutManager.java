...
import com.cliffc.aa.view.layout.LayoutGraph;
import com.cliffc.aa.view.layout.LayoutManager;
...
public class HierarchicalLayoutManager implements LayoutManager {
    ...
    private static final int SWEEP_ITERATIONS = 1;
    ...
    private static final int MAX_LAYER_LENGTH = -1;
    private static final int MIN_LAYER_DIFFERENCE = 1;
    private static final int VIP_BONUS = 10;
    ...
    private HashMap<Link, List<v2>> reversedLinkStartPoints;
    private HashMap<Link, List<v2>> reversedLinkEndPoints;
    private HashMap<LayoutEdge, LayoutEdge> bottomEdgeHash;
    private HashMap<Link, List<v2>> splitStartPoints;
    private HashMap<Link, List<v2>> splitEndPoints;
    ...
    private class LayoutNode {
        ...
        float yOffset;
        float bottomYOffset;
        Vertex vertex; // Only used for non-dummy nodes, otherwise null

        final List<LayoutEdge> preds = new ArrayList<>();
        final List<LayoutEdge> succs = new ArrayList<>();
        final HashMap<Float, Integer> outOffsets = new HashMap<>();
        final HashMap<Float, Integer> inOffsets = new HashMap<>();
        int pos = -1; // Position within layer
    ...
    public void doLayout(LayoutGraph graph, Set<? extends Link> importantLinks) {
        ...
        for (LayoutNode n : nodes) {
            ...
            for (LayoutEdge e : n.succs) if (importantLinks.contains(e.link)) tmpArr.add(e);
    ...
    private class WriteResult extends AlgorithmPart {
        ...
        protected void run() {
            ...
            HashMap<Vertex, v2> vertexPositions = new HashMap<>();
            HashMap<Link, List<v2>> linkPositions = new HashMap<>();
            for (Vertex v : graph.getVertices()) {
                LayoutNode n = vertexToLayoutNode.get(v);
                assert !vertexPositions.containsKey(v);
                vertexPositions.put(v, new v2(n.x + n.xOffset, n.y + n.yOffset));
            }
            ...
            for (LayoutNode n : nodes) {
                ...
                for (LayoutEdge e : n.preds)
                    if (e.link != null) {
                        ...
                        v2 p = new v2(e.to.x + e.relativeTo, e.to.y + e.to.yOffset + e.link.getTo().posRel().y);
                        points.add(p);
                        if (e.to.inOffsets.containsKey(e.relativeTo))
                            points.add(new v2(p.x, p.y + e.to.inOffsets.get(e.relativeTo) + e.link.getTo().posRel().y));
                        ...
                        while (cur.vertex == null && !cur.preds.isEmpty()) {
                            ...
                            points.add(new v2(cur.x + cur.width / 2, cur.y + cur.height));
                            if (points.size() > 1 && points.get(points.size() - 1).x == cur.x + cur.width / 2 && points.get(points.size() - 2).x == cur.x + cur.width / 2)
                                points.remove(points.size() - 1);
                            points.add(new v2(cur.x + cur.width / 2, cur.y));
                            assert cur.preds.size() == 1;
                            curEdge = cur.preds.get(0);
                            cur = curEdge.from;
                        }

                        p = new v2(cur.x + curEdge.relativeFrom, cur.y + cur.height - cur.bottomYOffset + (curEdge.link == null ? 0 : curEdge.link.getFrom().posRel().y));
                        if (curEdge.from.outOffsets.containsKey(curEdge.relativeFrom))
                            points.add(new v2(p.x, p.y + curEdge.from.outOffsets.get(curEdge.relativeFrom) + (curEdge.link == null ? 0 : curEdge.link.getFrom().posRel().y)));
                        points.add(p);
                        ...
                        if (cur.vertex == null && cur.preds.isEmpty()) {
                            ...
                            if (reversedLinkEndPoints.containsKey(e.link))
                                for (v2 p1 : reversedLinkEndPoints.get(e.link))
                                    points.add(new v2(p1.x + e.to.x, p1.y + e.to.y));
                            ...
                            if (splitStartPoints.containsKey(e.link)) {
                                ...
                                //checkPoints(points);
                                if (reversedLinks.contains(e.link)) Collections.reverse(points);
                                assert !linkPositions.containsKey(e.link);
                                linkPositions.put(e.link, points);
                            } else splitEndPoints.put(e.link, points);

                        } else {
                            if (reversedLinks.contains(e.link)) Collections.reverse(points);
                            if (reversedLinkStartPoints.containsKey(e.link))
                                for (v2 p1 : reversedLinkStartPoints.get(e.link))
                                    points.add(new v2(p1.x + cur.x, p1.y + cur.y));

                            if (reversedLinkEndPoints.containsKey(e.link))
                                for (v2 p1 : reversedLinkEndPoints.get(e.link))
                                    points.add(0, new v2(p1.x + other.x, p1.y + other.y));
                ...
                for (LayoutEdge e : n.succs)
                    if (e.link != null) {
                        ArrayList<v2> points = new ArrayList<>();
                        v2 p = new v2(e.from.x + e.relativeFrom, e.from.y + e.from.height - e.from.bottomYOffset + e.link.getFrom().posRel().y);
                        points.add(p);
                        if (e.from.outOffsets.containsKey(e.relativeFrom))
                            points.add(new v2(p.x, p.y + e.from.outOffsets.get(e.relativeFrom) + e.link.getFrom().posRel().y));
                        ...
                        p = new v2(cur.x + curEdge.relativeTo, cur.y + cur.yOffset + ((curEdge.link == null) ? 0 : curEdge.link.getTo().posRel().y));
                        points.add(p);
                        if (curEdge.to.inOffsets.containsKey(curEdge.relativeTo))
                            points.add(new v2(p.x, p.y + curEdge.to.inOffsets.get(curEdge.relativeTo) + ((curEdge.link == null) ? 0 : curEdge.link.getTo().posRel().y)));
                        ...
                        if (cur.succs.isEmpty() && cur.vertex == null) {
                            ...
                            if (splitEndPoints.containsKey(e.link)) {
                                ...
                                assert !linkPositions.containsKey(e.link);
                                linkPositions.put(e.link, points);
                            } else splitStartPoints.put(e.link, points);
                        } else {
            ...
            for (v2 p : vertexPositions.values()) {
                minX = Math.min(minX, p.x);
                minY = Math.min(minY, p.y);
            }

            for (List<v2> points : linkPositions.values())
                for (v2 p : points)
                    if (p != null) {
                        minX = Math.min(minX, p.x);
                        minY = Math.min(minY, p.y);
            ...
            for (Map.Entry<Vertex, v2> entry : vertexPositions.entrySet()) {
                v2 p = entry.getValue();
                p.x -= minX;
                p.y -= minY;
                entry.getKey().pos(p);
            ...
            for (Map.Entry<Link, List<v2>> entry : linkPositions.entrySet()) {
                List<v2> points = entry.getValue();
                for (v2 p : points)
                    if (p != null) {
                        p.x -= minX;
                        p.y -= minY;
                    }
                entry.getKey().setControlPoints(points);
        ...
        protected void printStatistics() {
            ...
            int edgeCount = 0;
            for (LayoutNode n : nodes) edgeCount += n.succs.size();
    ...
    private static final Comparator<Segment> segmentComparator = Comparator.comparingInt(s -> s.orderNumber);
    ...
    private static final Comparator<Region> regionComparator = Comparator.comparingInt(r -> r.minOrderNumber);
    ...
    private static final Comparator<LayoutNode> nodeProcessingDownComparator = (n1, n2) -> {
        int n1VIP = 0;
        for (LayoutEdge e : n1.preds) if (e.vip) n1VIP++;
        int n2VIP = 0;
        for (LayoutEdge e : n2.preds) if (e.vip) n2VIP++;
        if (n1VIP != n2VIP) return n2VIP - n1VIP;
        if (n1.vertex == null) {
            if (n2.vertex == null) return 0;
            return -1;
        }
    ...
    private static final Comparator<LayoutNode> nodeProcessingUpComparator = (n1, n2) -> {
        int n1VIP = 0;
        for (LayoutEdge e : n1.succs) if (e.vip) n1VIP++;
        int n2VIP = 0;
        for (LayoutEdge e : n2.succs) if (e.vip) n2VIP++;
        if (n1VIP != n2VIP) return n2VIP - n1VIP;
        if (n1.vertex == null) {
            if (n2.vertex == null) return 0;
            return -1;
    ...
    private class AssignXCoordinates extends AlgorithmPart {
        ...
        private void initialPositions() {
            for (LayoutNode n : nodes) n.x = space[n.layer].get(n.pos);
        }
        ...
        private void adjustSpace() {
            for (int i = 0; i < layers.length; i++) {
                ...
                int curX = 0;
                //                    curX += n.width + xOffset;
                for (LayoutNode n : layers[i]) space[i].add(n.x);
            }
        }

        private double calculateOptimalDown(LayoutNode n) {
            int size = n.preds.size();
            if (size == 0) return n.x;
            int vipCount = 0;
            for (LayoutEdge e : n.preds) if (e.vip) vipCount++;
            ...
            if (vipCount == 0) {
                ...
                int z = 0;
                float[] values = new float[vipCount];
                for (int i = 0; i < size; i++) {
                    LayoutEdge e = n.preds.get(i);
                    if (e.vip) values[z++] = e.from.x + e.relativeFrom - e.relativeTo;
                }
                return median(values);
            }
        }

        private double calculateOptimalBoth(LayoutNode n) {
            if (n.preds.size() == n.succs.size()) return n.x;

            float[] values = new float[n.preds.size() + n.succs.size()];
            int i = 0;
        ...
        private double calculateOptimalUp(LayoutNode n) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                values[i] = e.to.x + e.relativeTo - e.relativeFrom;
                if (e.vip) return values[i];
            }
            return median(values);
        }

        private double median(float[] values) {
            Arrays.sort(values);
        ...
        private void sweepUp(boolean onlyDummies) {
            for (int i = layers.length - 1; i >= 0; i--) {
                ...
                for (LayoutNode n : upProcessingOrder[i]) {
                    r.insert(n, (float) calculateOptimalUp(n));
                }
            }
        }
        ...
        private void doubleSweep() {
            for (int i = layers.length - 2; i >= 0; i--) {
                NodeRow r = new NodeRow(space[i]);
                for (LayoutNode n : upProcessingOrder[i]) {
                    r.insert(n, (float)calculateOptimalBoth(n));
                }
            }
        }
        ...
        private void sweepDown() {
            for (int i = 1; i < layers.length; i++) {
                ...
                for (LayoutNode n : downProcessingOrder[i]) {
                    r.insert(n, (float)calculateOptimalDown(n));
                }
            }
        }
    }
    ...
    private static class NodeRow {
        ...
        private final ArrayList<Float> space;

        NodeRow(ArrayList<Float> space) {
            treeSet = new TreeSet<>(nodePositionComparator);
            this.space = space;
        }

        float offset(LayoutNode n1, LayoutNode n2) {
            float v1 = space.get(n1.pos) + n1.width;
            float v2 = space.get(n2.pos);
            return v2 - v1;
        }

        void insert(LayoutNode n, float pos) {
    ...
    private class CrossingReduction extends AlgorithmPart {
        ...
        public void preCheck() {
            for (LayoutNode n : nodes) assert n.layer < layerCount;
        }
        ...
        private void createLayers() {
            ...
            for (int i = 0; i < layerCount; i++) layers[i] = new ArrayList<>();
        }
        ...
        private void initX() {
            ...
            for (int i = 0; i < layers.length; i++) updateXOfLayer(i);
        }
        ...
        private void downSweep() {
            ...
            for (int i = 1; i < layerCount; i++) {
                ...
                for (LayoutNode n : layers[i]) n.crossingNumber = 0;
                ...
                for (LayoutNode n : layers[i]) {
                    ...
                    int count = 0;
                    for (LayoutEdge e : n.preds) {
                        float cur = e.from.x + e.relativeFrom;
                        int factor = 1;
                        if (e.vip) factor = VIP_BONUS;
                        sum += cur * factor;
                        count += factor;
                    }
        ...
        private void updateCrossingNumbers(int index, boolean down) {
            for (int i = 0; i < layers[index].size(); i++) {
                ...
                if (i > 0) prev = layers[index].get(i - 1);
                LayoutNode next = null;
                if (i < layers[index].size() - 1) next = layers[index].get(i + 1);

                boolean cond = n.succs.isEmpty();
                if (down) cond = n.preds.isEmpty();

                if (cond) if (prev != null && next != null)
                    n.crossingNumber = (prev.crossingNumber + next.crossingNumber) / 2;
                else if (prev != null) n.crossingNumber = prev.crossingNumber;
                else if (next != null) n.crossingNumber = next.crossingNumber;
            }
        }
    ...
    private class CreateDummyNodes extends AlgorithmPart {
        ...
        protected void run() {
            ...
            if (combine == Combine.SAME_OUTPUTS) {
                ...
                HashMap<Float, List<LayoutEdge>> portHash = new HashMap<>();
                ...
                for (LayoutNode n : currentNodes) {
                    ...
                    HashMap<Float, LayoutNode> topNodeHash = new HashMap<>();
                    HashMap<Float, HashMap<Integer, LayoutNode>> bottomNodeHash = new HashMap<>();
                    for (LayoutEdge e : succs) {
                        ...
                        if (e.from.layer != e.to.layer - 1)
                            if (maxLayerLength != -1 && e.to.layer - e.from.layer > maxLayerLength/* && e.to.preds.size() > 1 && e.from.succs.size() > 1*/) {
                                ...
                                if (hash.containsKey(e.to.layer)) bottomNode = hash.get(e.to.layer);
                                else {
                                ...
                                float i = e.relativeFrom;
                                if (!portHash.containsKey(i)) portHash.put(i, new ArrayList<>());
                    ...
                    for (LayoutEdge e : succs) {
                        ...
                        if (portHash.containsKey(i)) {
                            ...
                            if (list.size() == 1) processSingleEdge(list.get(0));
                            else {
                ...
                ArrayList<LayoutNode> currentNodes = new ArrayList<>(nodes);
                for (LayoutNode n : currentNodes) for (LayoutEdge e : n.succs) processSingleEdge(e);
            }
        }
        ...
        private void processSingleEdge(LayoutEdge e) {
            ...
            if (e.to.layer > n.layer + 1) {
                LayoutEdge last = e;
                for (int i = n.layer + 1; i < last.to.layer; i++) last = addBetween(last, i);
            }
        }
        ...
        public void postCheck() {
            ArrayList<LayoutNode> currentNodes = new ArrayList<>(nodes);
            for (LayoutNode n : currentNodes) for (LayoutEdge e : n.succs) assert e.from.layer == e.to.layer - 1;

            for (int i = 0; i < layers.length; i++) {
                assert !layers[i].isEmpty();
                for (LayoutNode n : layers[i]) assert n.layer == i;
            }
        }
    }
    ...
    private class AssignLayers extends AlgorithmPart {
        ...
        @Override
        public void preCheck() {
            for (LayoutNode n : nodes) assert n.layer == -1;
        }
        ...
        private void assignLayerDownwards() {
            ...
            for (LayoutNode n : nodes) n.layer = (layerCount - 1 - n.layer);
        }

        private void assignLayerUpwards() {
            ArrayList<LayoutNode> hull = new ArrayList<>();
            for (LayoutNode n : nodes)
                if (n.succs.isEmpty()) hull.add(n);
                else n.layer = -1;
            ...
            while (!hull.isEmpty()) {
                ArrayList<LayoutNode> newSet = new ArrayList<>();
                for (LayoutNode n : hull)
                    if (n.layer < z) for (LayoutEdge se : n.preds) {
        ...
        public void postCheck() {
            for (LayoutNode n : nodes) {
                ...
                for (LayoutEdge e : n.succs) assert e.from.layer < e.to.layer;
            }
        }
    }
    ...
    private class ReverseEdges extends AlgorithmPart {
        ...
        protected void run() {
            ...
            for (LayoutNode node : nodes)
                if (node.vertex.isRoot()) {
                    ...
                    if (ok) reverseAllInputs(node);
                }

            // Start DFS and reverse back edges
            visited = new HashSet<>();
            active = new HashSet<>();
            for (LayoutNode node : nodes) DFS(node);

            for (LayoutNode node : nodes) {

                SortedSet<Float> reversedDown = new TreeSet<>();

                for (LayoutEdge e : node.succs) if (reversedLinks.contains(e.link)) reversedDown.add(e.relativeFrom);

                SortedSet<Float> reversedUp;
                if (reversedDown.isEmpty()) reversedUp = new TreeSet<>(Collections.reverseOrder());
                else reversedUp = new TreeSet<>();

                for (LayoutEdge e : node.preds) if (reversedLinks.contains(e.link)) reversedUp.add(e.relativeTo);

                final int offset = X_OFFSET + DUMMY_WIDTH;

                int curX = 0;
                float curWidth = node.width + reversedDown.size() * offset;
                for (float pos : reversedDown) {
                    ...
                    ArrayList<v2> startPoints = new ArrayList<>();
                    startPoints.add(new v2(curWidth, curX));
                    startPoints.add(new v2(pos, curX));
                    startPoints.add(new v2(pos, reversedDown.size() * offset));
                    for (LayoutEdge e : reversedSuccs) reversedLinkStartPoints.put(e.link, startPoints);
                ...
                curX = 0;
                int minX = 0;
                if (!reversedDown.isEmpty()) minX = -offset * reversedUp.size();

                float oldNodeHeight = node.height;
                for (float pos : reversedUp) {
                    ArrayList<LayoutEdge> reversedPreds = new ArrayList<>();
                    for (LayoutEdge e : node.preds)
                        if (e.relativeTo == pos && reversedLinks.contains(e.link)) {
                            if (reversedDown.isEmpty()) e.relativeTo = node.width + offset;
                            else e.relativeTo = curX - offset;

                            reversedPreds.add(e);
                        }
                    node.height += offset;
                    ArrayList<v2> endPoints = new ArrayList<>();

                    if (reversedDown.isEmpty()) {

                        curX += offset;
                        node.width += offset;
                        endPoints.add(new v2(node.width, node.height));

                    } else {
                        curX -= offset;
                        node.width += offset;
                        endPoints.add(new v2(curX, node.height));
                    }
                    ...
                    endPoints.add(new v2(pos, node.height));
                    endPoints.add(new v2(pos, oldNodeHeight));
                    for (LayoutEdge e : reversedPreds) reversedLinkEndPoints.put(e.link, endPoints);
                }

                if (minX < 0) {
                    for (LayoutEdge e : node.preds) e.relativeTo -= minX;

                    for (LayoutEdge e : node.succs) e.relativeFrom -= minX;
        ...
        private void reverseAllInputs(LayoutNode node) {
            for (LayoutEdge e : node.preds) {
                ...
                e.from.preds.add(e);
                e.from.succs.remove(e);
                float oldRelativeFrom = e.relativeFrom;
                float oldRelativeTo = e.relativeTo;
                e.to = e.from;
                e.from = node;
    ...
    private final Comparator<Link> linkComparator = (l1, l2) -> {
        if (l1.isVIP() && !l2.isVIP()) return -1;

        if (!l1.isVIP() && l2.isVIP()) return 1;

        int result = l1.getFrom().v().compareTo(l2.getFrom().v());
        if (result != 0) return result;
        result = l1.getTo().v().compareTo(l2.getTo().v());
        if (result != 0) return result;
        result = Float.compare(l1.getFrom().posRel().x , l2.getFrom().posRel().x);
        if (result != 0) return result;
        return Float.compare(l1.getTo().posRel().x , l2.getTo().posRel().x);
    };
    ...
    private class BuildDatastructure extends AlgorithmPart {
        ...
        protected void run() {
            ...
            for (Link l : links) {
                ...
                assert vertexToLayoutNode.containsKey(l.getFrom().v());
                assert vertexToLayoutNode.containsKey(l.getTo().v());
                edge.from = vertexToLayoutNode.get(l.getFrom().v());
                edge.to = vertexToLayoutNode.get(l.getTo().v());
                edge.relativeFrom = l.getFrom().posRel().x;
                edge.relativeTo = l.getTo().posRel().x;
                edge.link = l;
                edge.vip = l.isVIP();
            ...
            for (Link l : importantLinks) {
                if (!vertexToLayoutNode.containsKey(l.getFrom().v())
                        || vertexToLayoutNode.containsKey(l.getTo().v())) continue;
                LayoutNode from = vertexToLayoutNode.get(l.getFrom().v());
                LayoutNode to = vertexToLayoutNode.get(l.getTo().v());
                for (LayoutEdge e : from.succs) if (e.to == to) linksToFollow.add(e.link);
            }
        }
        ...
        public void postCheck() {
            ...
            for (Vertex v : graph.getVertices()) {
                ...
                for (LayoutEdge e : node.succs) assert e.from == node;

                for (LayoutEdge e : node.preds) assert e.to == node;
